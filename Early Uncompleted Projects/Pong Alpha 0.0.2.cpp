...
public:
	...
	sf::Sprite BallImage;
	Ball(sf::Vector2f position, float radius, sf::Color color, sf::Texture &texture)
	{
...
int main()
{
	...
	BackgroundTexture.loadFromFile("backgroundman.png");
	...
	PlayerTexture.loadFromFile("paddleblu.png");

	Player player1(Player(pPosition1, pSize, pColor, PlayerTexture)); //creating the player
	player1.rect.setOutlineThickness(2);
	player1.rect.setOutlineColor(sf::Color::Black);

	Player player2(Player(pPosition2, pSize, pColor, PlayerTexture));
	player2.rect.setOutlineThickness(2);
	player2.rect.setOutlineColor(sf::Color::Black);
	...
	BallTexture.loadFromFile("ballGrey.png");

	Ball ball(Ball(bPosition, bRadius, bColor, BallTexture)); //creating the ball
	ball.circ.setOutlineThickness(2);
	ball.circ.setOutlineColor(sf::Color::Black);
	...
	while(Window.isOpen()) //game loop
	{
		...
		while(Window.pollEvent(Event)) //event loop
		{
			...
			case sf::Event::MouseButtonPressed:
				...
				ball.BallImage.setPosition(bPosition);
				break;
			}
		}
		...
		if(player1.CanMove(player1)) //moving player 1
		{
			if(sf::Keyboard::isKeyPressed(sf::Keyboard::W))
			{
				...
				player1.PlayerImage.move( 0.0f, -paddleMovement );
			}
		...
		if(player2.CanMove(player2)) //moving player 2
		{
			if(sf::Keyboard::isKeyPressed(sf::Keyboard::Up))
			{
				...
				player2.PlayerImage.move(0.0f, -paddleMovement);
			}
		...
		if(ballVelX == 1) //making the ball move around the screen when not hitting
		{
			...
			ball.BallImage.move(ballMovementX, ballMovementY);
		}
		else if(ballVelX == -1)
		{
			ball.circ.move(-ballMovementX, ballMovementY);
